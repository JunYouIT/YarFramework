...
$.fn.bic_calendar = function(options) {
    ...
    this.each(function() {
        ...
        function showMonthDays(month, year) {
            ...
            textMonthCurrentLayer.text(year + ' ' + monthNames[month]);
            ...
            var firstDay = calcNumberDayWeek(1, month, year)+1;
            ...
            for (var i = 0; i < 7; i++) {
                if (i < firstDay) {
                    ...
                    dayCode += '><div><a class="daylink" data-date="' + nMonth + "/" + daysCounter + "/" + year + '" ' + '>' + daysCounter + '</a></div></span>';
                    if (i == 6)
                        dayCode += '</tr>';
                    daysCounter++;
                }
        ...
        function checkEvents(month, year) {
			if(opts.monthChange){
				opts.monthChange(month+1,year,function(eventdata){
						markEvents(month, year,eventdata);					
				});
         ...
         */
        function markEvents(month, year,eventdata) {
            ...
            for (var i = 0; i < events.length; i++) {
                ...
                if (events[i].date.split('/')[1] == temporalMonth && events[i].date.split('/')[2] == year) {
					...
					loopDayA.attr('data-date',events[i].date);
                    //bg color
                    if (events[i].color)
					...
					loopDayA.click(function(ev){						
						if(opts.eventDayClick){
							var date = $(this).data('date');
							opts.eventDayClick(date,ev);
						}
					});
					/* 
					else {
			...
			if(eventdata && eventdata.length > 0){
                ...
                if (eventdata[i].date.split('/')[1] == temporalMonth && eventdata[i].date.split('/')[2] == year) {

                    var loopDayTd = $('#' + calendarId + '_' + eventdata[i].date.replace(/\//g, "_"));
                    var loopDayA = $('#' + calendarId + '_' + eventdata[i].date.replace(/\//g, "_") + ' a');

                    loopDayTd.addClass('event');

                    loopDayA.attr('data-original-title', eventdata[i].title);
					loopDayA.attr('data-date',eventdata[i].date);
                    //bg color
                    if (eventdata[i].color)
                        loopDayTd.css('background', eventdata[i].color);

                    //link
                    if (typeof eventdata[i].link != 'undefined' && eventdata[i].link != '') {
                        loopDayA.attr('href', eventdata[i].link);
                    }
                    if (typeof eventdata[i].linkTarget != 'undefined' && eventdata[i].linkTarget != '') {
                        loopDayA.attr('target', eventdata[i].linkTarget);
                    }

                    //class
                    if (eventdata[i].class)
                        loopDayTd.addClass(eventdata[i].class);

                    //tooltip vs popover
                    if (eventdata[i].content) {
                        loopDayTd.addClass('event_popover');
                        loopDayA.attr('rel', 'popover');
                        loopDayA.attr('data-content', eventdata[i].content);
                    }
					loopDayA.click(function(ev){						
						if(opts.eventDayClick){
							var date = $(this).data('date');
